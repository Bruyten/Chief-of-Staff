...
const schema = z
  .object({
    ...
    VIDEO_POLL_BATCH_SIZE: z.coerce.number().int().positive().max(100).default(10),
  })
  .superRefine((value, ctx) => {
    if (value.NODE_ENV === "production" && !value.FAKE_AI && !value.OPENAI_API_KEY) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["OPENAI_API_KEY"],
        message: "OPENAI_API_KEY is required when FAKE_AI is false in production",
      });
    }
    ...
    if (value.NODE_ENV === "production" && value.VIDEO_PROVIDER !== "mock") {
      if (!value.VIDEO_PROVIDER_API_KEY) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: ["VIDEO_PROVIDER_API_KEY"],
          message: "VIDEO_PROVIDER_API_KEY is required for live video providers",
        });
      }
    }
  });
