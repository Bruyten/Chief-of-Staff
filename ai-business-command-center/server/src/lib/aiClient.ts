...
const CSRF_STORAGE_KEY = "cos_csrf_header_token";
...
function setStoredCsrfToken(token: string | null | undefined) {
  if (!token) {
...
async function fetchFreshCsrfToken() {
  ...
  if (!response.ok) {
    throw new ApiException({
      ...
      message: "Could not refresh the security token.",
    });
  }
  ...
  if (!payload.token) {
    throw new ApiException({
      ...
      message: "The server did not return a security token.",
    });
  }
...
export const auth = {
  signup: async (data: {
    ...
    const response = await api<{ user: User; csrfToken?: string }>(
      "/api/auth/signup",
      {
        method: "POST",
        body: data,
    ...
    if (response.csrfToken) {
      setStoredCsrfToken(response.csrfToken);
    }
  ...
  logout: async () => {
    const response = await api<{ ok: true }>("/api/auth/logout", {
      method: "POST",
    });
...
export const brandVoices = {
  ...
  create: (
    data: Partial<BrandVoiceProfile> & {
      brandName: string;
    },
  ) =>
