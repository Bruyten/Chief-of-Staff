...
import { billingSpec, type BillBlock } from "../data/billing";
import { cn } from "../utils/cn";

type Props = { activeSection: string; onVisibleChange: (id: string) => void };

export default function BillingView({ activeSection, onVisibleChange }: Props) {
  const containerRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    const el = document.getElementById(`bill-section-${activeSection}`);
  ...
  useEffect(() => {
    const observer = new IntersectionObserver(
      (entries) => {
        ...
        if (visible) onVisibleChange(visible.target.id.replace("bill-section-", ""));
      },
      { rootMargin: "-20% 0px -60% 0px", threshold: [0, 0.25, 0.5, 1] }
    );
    billingSpec.forEach((s) => {
      const el = document.getElementById(`bill-section-${s.id}`);
  ...
  return (
    <div ref={containerRef} className="flex-1 overflow-y-auto">
      <div className="relative overflow-hidden border-b border-white/5">
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top,_rgba(99,102,241,0.14),_transparent_60%)]" />
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_bottom_right,_rgba(34,197,94,0.1),_transparent_50%)]" />
        <div className="relative max-w-4xl mx-auto px-10 pt-16 pb-14">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-500/10 border border-indigo-400/20 text-[11px] text-indigo-200 uppercase tracking-widest mb-6">
          ...
          <p className="text-white/60 text-lg mt-5 max-w-2xl leading-relaxed">
            Free / Starter / Pro / Agency. Stripe Checkout, customer portal,
            webhook handler, plan-aware credits — all live in the App tab right
            now via FAKE_STRIPE mode. Flip one env var to go live.
          </p>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-8">
            {[
              { label: "Plans", value: "4" },
              { label: "Routes", value: "5" },
              { label: "Webhook events", value: "4" },
              { label: "Test items", value: "12" },
            ].map((s) => (
      ...
      <div className="max-w-4xl mx-auto px-10 py-12 space-y-20">
        {billingSpec.map((section) => (
          <section key={section.id} id={`bill-section-${section.id}`} className="scroll-mt-8">
            <div className="flex items-baseline gap-4 mb-2">
              <span className="text-indigo-300/60 font-mono text-xs">{section.number}</span>
        ...
        <div className="ml-12 mt-16 pt-10 border-t border-white/5">
          <div className="rounded-2xl bg-gradient-to-br from-indigo-500/15 via-blue-500/10 to-emerald-500/15 border border-white/10 p-8">
            <div className="text-2xl mb-2">🎉</div>
            <h3 className="text-xl font-bold text-white mb-2">MVP fully complete.</h3>
            <p className="text-white/60 leading-relaxed">
              Plan ✓ MVP ✓ Arch ✓ Prompts ✓ Templates ✓ Backend ✓ Wiring ✓ Library ✓ Billing ✓.
              You now have a real, paying-customer-ready SaaS. Suggested final prompts:
            </p>
            <ul className="mt-3 space-y-2 text-white/70 text-[14px]">
              ...
              <li>→ <span className="text-white">"Generate the final root README with quickstart, architecture, screenshots"</span></li>
...
function BlockRenderer({ block }: { block: BillBlock }) {
  switch (block.type) {
      ...
      <ul className="space-y-2">
        {block.items.map((item, i) => (
          <li key={i} className="flex gap-3 text-white/75 text-[15px] leading-relaxed">
            <span className="text-indigo-400 mt-1.5 shrink-0">•</span><span>{item}</span>
          </li>
        ))}
      </ul>
    );
    case "ordered": return (
      <ol className="space-y-2.5">
        {block.items.map((item, i) => (
          <li key={i} className="flex gap-3 text-white/75 text-[15px] leading-relaxed">
            <span className="shrink-0 w-6 h-6 rounded-md bg-white/5 border border-white/10 text-[11px] text-indigo-300 grid place-items-center font-mono mt-0.5">{i + 1}</span>
            <span>{item}</span>
          </li>
        ))}
      </ol>
...
function FileCodeBlock({ path, lang, description, code }: { path: string; lang: string; description: string; code: string }) {
  ...
  return (
    <div className="rounded-xl bg-[#0a0b0f] border border-indigo-400/20 overflow-hidden">
      <button onClick={() => setOpen(!open)} className="w-full text-left px-4 py-3 border-b border-white/5 bg-gradient-to-r from-indigo-500/[0.06] to-transparent hover:from-indigo-500/[0.1] transition flex items-center gap-3">
        <div className="text-indigo-300 text-base">📄</div>
        <div className="flex-1 min-w-0">
          <code className="text-indigo-200 font-mono text-[12.5px] block truncate">{path}</code>
