...
import { backendSpec, type BackendBlock } from "../data/backend";
...
export default function BackendView({ activeSection, onVisibleChange }: Props) {
  const containerRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    const el = document.getElementById(`backend-section-${activeSection}`);
  ...
  useEffect(() => {
    const observer = new IntersectionObserver(
      (entries) => {
        ...
        if (visible) {
          const id = visible.target.id.replace("backend-section-", "");
          onVisibleChange(id);
        }
      },
      { rootMargin: "-20% 0px -60% 0px", threshold: [0, 0.25, 0.5, 1] }
    );
    backendSpec.forEach((s) => {
      const el = document.getElementById(`backend-section-${s.id}`);
  ...
  return (
    <div ref={containerRef} className="flex-1 overflow-y-auto">
      <div className="relative overflow-hidden border-b border-white/5">
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top,_rgba(34,197,94,0.14),_transparent_60%)]" />
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_bottom_right,_rgba(56,189,248,0.1),_transparent_50%)]" />
        <div className="relative max-w-4xl mx-auto px-10 pt-16 pb-14">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-green-500/10 border border-green-400/20 text-[11px] text-green-200 uppercase tracking-widest mb-6">
          ...
          <p className="text-white/60 text-lg mt-5 max-w-2xl leading-relaxed">
            Production-ready Express + TypeScript + Prisma + OpenAI backend.
            45 files, ~1500 lines, runs locally in under 10 minutes — no
            API key needed thanks to FAKE_AI mode.
          </p>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-8">
            {[
              { label: "API routes", value: "22" },
              { label: "DB tables", value: "5" },
              { label: "Prompt files", value: "16" },
              { label: "Files shipped", value: "45+" },
            ].map((s) => (
      ...
      <div className="max-w-4xl mx-auto px-10 py-12 space-y-20">
        {backendSpec.map((section) => (
          <section key={section.id} id={`backend-section-${section.id}`} className="scroll-mt-8">
            <div className="flex items-baseline gap-4 mb-2">
              <span className="text-green-300/60 font-mono text-xs">{section.number}</span>
        ...
        <div className="ml-12 mt-16 pt-10 border-t border-white/5">
          <div className="rounded-2xl bg-gradient-to-br from-green-500/15 via-emerald-500/10 to-teal-500/15 border border-white/10 p-8">
            <div className="text-2xl mb-2">🔌</div>
            <h3 className="text-xl font-bold text-white mb-2">Backend shipped. Time to wire the frontend.</h3>
            <p className="text-white/60 leading-relaxed">
              The /server folder is real and ready. Suggested next prompts:
            </p>
            <ul className="mt-3 space-y-2 text-white/70 text-[14px]">
              <li>→ <span className="text-white">"Wire the frontend to the real backend — replace mocked AppContext with fetch calls"</span></li>
              ...
              <li>→ <span className="text-white">"Add Stripe billing for Pro tier"</span></li>
            </ul>
          </div>
        </div>
      </div>
...
function BlockRenderer({ block }: { block: BackendBlock }) {
  switch (block.type) {
    case "p":
    ...
    case "ordered":
      return (
        <ol className="space-y-2.5">
          {block.items.map((item, i) => (
            <li key={i} className="flex gap-3 text-white/75 text-[15px] leading-relaxed">
              <span className="shrink-0 w-6 h-6 rounded-md bg-white/5 border border-white/10 text-[11px] text-green-300 grid place-items-center font-mono mt-0.5">
                {i + 1}
              </span>
              <span>{item}</span>
            </li>
      ...
      return (
        <div className="rounded-xl bg-[#0a0b0f] border border-white/10 overflow-hidden">
          ...
          <pre className="p-4 overflow-x-auto text-[12.5px] leading-relaxed text-white/85 font-mono">
            <span className="text-green-300">{block.root}</span>
            {"\n"}
            {block.lines.join("\n")}
          </pre>
        </div>
              ...
              {block.rows.map((r, i) => {
                ...
                return (
                  <tr key={i} className="border-t border-white/5 align-top">
                    ...
                    <td className="px-4 py-2.5"><code className="text-white/85 font-mono text-[12.5px]">{r.path}</code></td>
                    <td className="px-4 py-2.5">
                      {r.auth ? (
                        <span className="text-[10px] uppercase tracking-wider text-violet-200 bg-violet-500/10 border border-violet-400/20 px-1.5 py-0.5 rounded">🔒 yes</span>
                      ) : (
                        <span className="text-[10px] uppercase tracking-wider text-white/40 bg-white/5 border border-white/10 px-1.5 py-0.5 rounded">public</span>
                      )}
                    </td>
          ...
          <table className="w-full text-[13px]">
            ...
            <tbody>
              {block.rows.map((r, i) => (
                <tr key={i} className="border-t border-white/5 align-top">
                  <td className="px-4 py-2.5">
                    <code className="text-green-200 font-mono text-[12.5px]">{r.key}</code>
                    {r.secret && (
    ...
    <div className="rounded-xl bg-[#0a0b0f] border border-white/10 overflow-hidden">
      <div className="flex items-center justify-between px-4 py-2 border-b border-white/5 bg-white/[0.02]">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={onCopy} className="text-[10px] uppercase tracking-widest text-white/50 hover:text-white px-2 py-0.5 rounded border border-white/10 hover:border-white/20 transition">
            {copied ? "copied" : "copy"}
          </button>
        </div>
      </div>
...
function FileCodeBlock({ path, lang, description, code }: { path: string; lang: string; description: string; code: string }) {
  ...
  return (
    <div className="rounded-xl bg-[#0a0b0f] border border-green-400/20 overflow-hidden">
      <button onClick={() => setOpen(!open)} className="w-full text-left px-4 py-3 border-b border-white/5 bg-gradient-to-r from-green-500/[0.06] to-transparent hover:from-green-500/[0.1] transition flex items-center gap-3">
        <div className="text-green-300 text-base">📄</div>
        <div className="flex-1 min-w-0">
          <code className="text-green-200 font-mono text-[12.5px] block truncate">{path}</code>
